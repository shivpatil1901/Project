...
# def load_data(csv_path):
#     # Use efficient dtypes on load to reduce memory from the start
...
#     }
#     df['loan_status_binary'] = df['loan_status'].map(loan_status_mapping)
#     unmapped = df['loan_status_binary'].isna().sum()
#     if unmapped > 0:
#         df = df[df['loan_status_binary'].notna()]
#         print(f"Dropped {unmapped} rows with unmapped loan_status values")

#     df['loan_status_binary'] = df['loan_status_binary'].astype(np.int8)  # int8 not int64
...
# def process_categorical_and_dates(df):
#     # Strip whitespace — inplace per column to avoid full copy
...
#                      ('last_pymnt_d', '%b-%Y'), ('last_credit_pull_d', '%b-%Y')]:
...
#             df['int_rate%'] = pd.to_numeric(df['int_rate'].str.strip('%'), errors='coerce').astype(np.float32)
#         else:
#             df['int_rate%'] = df['int_rate'].astype(np.float32)

#     if 'revol_util' in df.columns:
#         if df['revol_util'].dtype == object:
#             df['revol_util%'] = pd.to_numeric(df['revol_util'].str.strip('%'), errors='coerce').astype(np.float32)
#         else:
#             df['revol_util%'] = df['revol_util'].astype(np.float32)


#     # Binary flags
#     if 'debt_settlement_flag' in df.columns:
#         df['debt_settlement_flag'] = (df['debt_settlement_flag'] == 'Y').astype(np.int8)
#     if 'term' in df.columns:
#         df['term_36_months'] = (df['term'] == '36 months').astype(np.int8)
...
#         df['hardship_flag'] = df['hardship_flag'].fillna('N')
#         df['hardship_flag'] = (df['hardship_flag'] == 'Y').astype(np.int8)

#     print("Processed categorical and date features")
#     return df
...
#     df['loan_age'] = (today - df['issue_d']).dt.days.astype(np.float32)
#     df['credit_history_length'] = (df['issue_d'] - df['earliest_cr_line']).dt.days.astype(np.float32)
#     df['time_since_last_payment'] = (today - df['last_pymnt_d']).dt.days.astype(np.float32)
#     df['time_since_last_credit_pull'] = (today - df['last_credit_pull_d']).dt.days.astype(np.float32)

#     df['issue_year'] = df['issue_d'].dt.year.astype(np.int16)
#     df['issue_month'] = df['issue_d'].dt.month.astype(np.int8)

#     df['recent_payment'] = (df['time_since_last_payment'] <= 30).astype(np.int8)
#     df['recent_credit_pull'] = (df['time_since_last_credit_pull'] <= 90).astype(np.int8)

#     df.drop(columns=['issue_d', 'earliest_cr_line', 'last_pymnt_d', 'last_credit_pull_d'],
#             inplace=True)
...
# def remove_time_related_features(df):
#     time_cols = ['loan_age', 'credit_history_length', 'time_since_last_payment',
#                  'time_since_last_credit_pull', 'issue_year', 'issue_month',
#                  'recent_payment', 'recent_credit_pull']
#     existing = [c for c in time_cols if c in df.columns]
#     df.drop(columns=existing, inplace=True)
#     print(f"Removed {len(existing)} time-related features")
#     return df


# def prepare_numeric_features(df):
#     df.drop(columns=['sub_grade'], inplace=True, errors='ignore')
#     df.drop(columns=['application_type', 'initial_list_status',
#                      'addr_state', 'purpose'], inplace=True, errors='ignore')

#     if 'home_ownership' in df.columns:
#         mapping = {'ANY': 'other', 'MORTGAGE': 'other', 'RENT': 'rent',
#                    'OWN': 'own', 'NONE': 'other'}
#         df['home_ownership'] = df['home_ownership'].map(mapping)

#     y = df.pop('loan_status_binary')  # pop instead of copy — avoids duplication

#     cat_cols = df.select_dtypes(include='object').columns.tolist()
#     num_cols = df.select_dtypes(exclude='object').columns.tolist()

#     # One-hot encode with uint8 dtype — much smaller than default bool/int64
#     df_dummies = pd.get_dummies(df[cat_cols], drop_first=True, dtype=np.uint8)

#     # Keep only numeric, drop original cat cols from df to free memory
#     df_num = df[num_cols].copy()
#     del df  # free original dataframe immediately

#     df_final = pd.concat([df_num, df_dummies], axis=1)
#     del df_num, df_dummies  # free intermediates immediately

#     # Drop external scores and id
#     drop_cols = ['last_fico_range_high', 'last_fico_range_low',
#                  'fico_range_low', 'fico_range_high', 'id']
#     df_final.drop(columns=drop_cols, inplace=True, errors='ignore')

#     # Impute — use float32 output to halve memory vs float64
#     imputer = SimpleImputer(strategy='mean')
#     X_imputed = imputer.fit_transform(df_final).astype(np.float32)
#     df_final = pd.DataFrame(X_imputed, columns=df_final.columns)
#     del X_imputed  # free numpy array immediately
...
#     df = remove_time_related_features(df)
#     df_final, y = prepare_numeric_features(df)
#     del df  # ensure original is freed before split
